...
namespace WEBUIautomation.WebElement
{
    public partial class WebElement : ICloneable
    {
        ...
        private WebElementNotFoundException WebElementNotFoundException
        {
            get
            {
                return new WebElementNotFoundException(string.Format("Can't find single element with given search criteria: {0}. Current driver frame is {1} , Current driver view is {2}",
                    SearchCriteriaToString(), Browser.CurrentFrame.ToString(), Browser.CurrentView));
            }
        }
